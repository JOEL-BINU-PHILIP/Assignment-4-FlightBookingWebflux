...
public class BookingServiceImplTest {
    ...
    void testBookTicket_success() {
        ...
        StepVerifier.create(bookingService.bookTicket(flight.getId(), req))
                .expectNextMatches(resp -> resp.getFlightId().equals(flight.getId()))
                .verifyComplete();
    }
    ...
    void testBookTicket_flightNotFound() {
        ...
        StepVerifier.create(bookingService.bookTicket("badId", req))
    ...
    void testBookTicket_duplicateSeats() {
        PassengerRequest p1 = PassengerRequest.builder()
                .name("A").gender("M").age(20)
                .seatNumber("1A").meal("veg").build();

        PassengerRequest p2 = PassengerRequest.builder()
                .name("B").gender("M").age(22)
                .seatNumber("1A").meal("veg").build(); // duplicate seat
    ...
    void testGetTicketByPnr_success() {
        ...
        when(passengerRepository.findByBookingId(booking.getId()))
                .thenReturn(Flux.just(passenger));

        StepVerifier.create(bookingService.getTicketByPnr("PNR12345"))
                .expectNextMatches(resp -> resp.getPnr().equals("PNR12345"))
                .verifyComplete();
    }
    ...
    void testGetTicketByPnr_notFound() {
        ...
        StepVerifier.create(bookingService.getTicketByPnr("bad"))
                .expectError(ApiException.class)
                .verify();
    }
    ...
    void testCancelBooking_success() {
        ...
        when(bookingRepository.findByPnr(booking.getPnr()))
                .thenReturn(Mono.just(booking));

        when(flightRepository.findById(booking.getFlightId()))
                .thenReturn(Mono.just(flight));

        when(bookingRepository.save(any())).thenReturn(Mono.just(booking));
        when(flightRepository.save(any())).thenReturn(Mono.just(flight));

        StepVerifier.create(bookingService.cancelBooking(booking.getPnr()))
                .verifyComplete();
    }
    ...
    void testCancelBooking_notFound() {
        ...
        StepVerifier.create(bookingService.cancelBooking("bad"))
                .expectError(ApiException.class)
                .verify();
    }
